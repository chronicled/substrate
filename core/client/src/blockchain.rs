...
/// 
/// Note: this implementation seems to work well with our current query pattern:
/// lca(best, final) (O(h)), lca(best + 1, final) (O(1)), lca(best + 2, final) (O(1)), etc.
/// 
/// TODO: split the tree in sections and use `ancestor` links to jump between them.
...
pub fn tree_route<Block: BlockT, Backend: HeaderBackend<Block>>(
	...
	let get_light_header = |id: BlockId<Block>| {
		match backend.get_light_header(id) {
			Ok(Some(hdr)) => Ok(hdr),
