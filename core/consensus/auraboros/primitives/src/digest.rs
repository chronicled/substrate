...
#[cfg(feature = "std")]
use super::{AURABOROS_ENGINE_ID, Epoch};
#[cfg(not(feature = "std"))]
...
#[derive(Clone, Encode, Decode)]
pub enum RawAuraborosPreDigest {
	Aura(AuraPreDigest),
	Babe(RawBabePreDigest),
}
...
pub trait CompatibleDigestItem: Sized {
	...
	fn auraboros_pre_digest(seal: AuraBorosPreDigest) -> Self;

	/// If this item is an BABE pre-digest, return it.
	fn as_auraboros_pre_digest(&self) -> Option<AuraBorosPreDigest>;

	/// Construct a digest item which contains a BABE seal.
	fn auraboros_seal(signature: Signature) -> Self;

	/// If this item is a BABE signature, return the signature.
	fn as_auraboros_seal(&self) -> Option<Signature>;
...
{
	fn auraboros_pre_digest(digest: AuraBorosPreDigest) -> Self {
		DigestItem::PreRuntime(AURABOROS_ENGINE_ID, digest.encode())
	}

	fn as_auraboros_pre_digest(&self) -> Option<AuraBorosPreDigest> {
		self.try_to(OpaqueDigestItemId::PreRuntime(&AURABOROS_ENGINE_ID))
	}

	fn auraboros_seal(signature: Signature) -> Self {
		DigestItem::Seal(AURABOROS_ENGINE_ID, signature.encode())
	}

	fn as_auraboros_seal(&self) -> Option<Signature> {
		self.try_to(OpaqueDigestItemId::Seal(&AURABOROS_ENGINE_ID))
	}

	fn as_babe_epoch(&self) -> Option<Epoch> {
		self.try_to(OpaqueDigestItemId::Consensus(&AURABOROS_ENGINE_ID))
	}
}
