	...
	Error: std::error::Error + Send + From<::consensus_common::Error> + From<I::Error> + 'static,
{
	...
	fn on_slot(
		...
		// TODO: seed local rng with VRF output from last babe block
		let proposal_work = if let Some(inherent_digest) = claim_slot(
			...
			futures::future::select(
				proposer.propose(
					...
					generic::Digest {
						logs: vec![
							generic::DigestItem::auraboros_pre_digest(inherent_digest.clone()),
						],
					},
					remaining_duration,
		...
		Box::pin(proposal_work.map_ok(move |b| {
			...
			let signature_digest_item = DigestItemFor::<B>::auraboros_seal(signature);
...
fn find_pre_digest<B: BlockT>(header: &B::Header) -> Result<AuraBorosPreDigest, String> {
	...
	for log in header.digest().logs() {
		...
		match (log.as_auraboros_pre_digest(), pre_digest.is_some()) {
	...
	where DigestItemFor<B>: CompatibleDigestItem,
{
	...
	for log in header.digest().logs() {
		...
		let log = log.try_to::<ConsensusLog>(OpaqueDigestItemId::Consensus(&AURABOROS_ENGINE_ID));
	...
	where DigestItemFor<B>: CompatibleDigestItem,
{
	...
	let sig = seal.as_auraboros_seal().ok_or_else(|| {
		babe_err!("Header {:?} has a bad seal", hash)
	})?;

	let pre_digest = find_pre_digest::<B>(&header)?;
	let slot_num = pre_digest.slot_number();

	if slot_num > slot_now {
		header.digest_mut().push(seal);
		return Ok(CheckedHeader::Deferred(header, slot_num));
	}
	...
	let pre_digest = CompatibleDigestItem::auraboros_pre_digest(pre_digest);
...
fn check_babe_header<B: BlockT + Sized, C: AuxStore>(
	client: &C,
	pre_digest: &BabePreDigest,
	slot_now: u64,
	sig: Signature,
	header: &B::Header,
	hash: B::Hash,
	...
	C: client::backend::AuxStore,
{
	...
	if sr25519::Pair::verify(&sig, pre_hash.as_ref(), expected_author) {
		if let Some(equivocation_proof) = check_equivocation(
			client,
			slot_now,
			*slot_number,
	...
	fn verify(
		...
		match checked_header {
			CheckedHeader::Checked(pre_header, (pre_digest, seal)) => {
				let slot_number = pre_digest.as_auraboros_pre_digest()
...
fn make_transcript(
	...
	let mut transcript = Transcript::new(&AURABOROS_ENGINE_ID);
...
fn aura_slot_author(slot_number: u64, authorities: &[AuthorityId]) -> Option<&AuthorityId> {
