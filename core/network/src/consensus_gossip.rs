...
impl<'g, 'p, B: BlockT> ValidatorContext<'g, 'p, B> {
	...
	pub fn broadcast_topic(&mut self, topic: B::Hash) {
	   let messages: Vec<_> = self.gossip.messages.iter()
		   ...
		   .collect();
		for m in messages {
		   self.gossip.multicast(self.protocol, topic, m, false);
		}
	}
...
impl<B: BlockT> ConsensusGossip<B> {
		...
		where F: Fn() -> ConsensusMessage,
	{
		for (id, ref mut peer) in self.peers.iter_mut() {
			if peer.known_messages.contains(&message_hash) && !force {
				continue
			}
	...
	pub fn on_incoming(
		...
		if let Some(ref mut peer) = self.peers.get_mut(&who) {
			...
			self.multicast_inner(protocol, message_hash, topic, || message.clone(), false);
			Some((topic, message))
		} else {
