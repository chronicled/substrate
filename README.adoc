...
2. Using https://polkadot.js.org/apps/next/#/settings[polkadot.js.org] you can store any `Cid` (it's `Bytes` representation) on-chain. Just navigate to `Extrinsics` and `sudo` a `Consensus::setApp`-proposal. This will set the given bytes as the value for the key `:app`. 
