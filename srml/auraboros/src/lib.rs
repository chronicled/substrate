...
use auraboros_primitives::{AURABOROS_ENGINE_ID, ConsensusLog, BabeWeight, Epoch, RawAuraborosPreDigest};
pub use auraboros_primitives::{AuthorityId, VRF_OUTPUT_LENGTH, PUBLIC_KEY_LENGTH};
use auraboros_primitives::{RawBabePreDigest, AuraPreDigest};
	...
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		fn on_initialize() {
			for digest in Self::get_inherent_digests()
				...
				.filter_map(|(id, mut data)| if id == AURABOROS_ENGINE_ID {
					RawAuraborosPreDigest::decode(&mut data).ok()
				} else {
					None
				})
			{
				...
				if let RawAuraborosPreDigest::Babe(RawBabePreDigest { vrf_output, .. }) = digest {
...
impl<T: Trait> FindAuthor<u32> for Module<T> {
		...
		I: 'a + IntoIterator<Item=(ConsensusEngineId, &'a [u8])>
	{
		for (id, mut data) in digests.into_iter() {
			if id == AURABOROS_ENGINE_ID {
				let pre_digest = RawAuraborosPreDigest::decode(&mut data).ok()?;
				return Some(match pre_digest {
					RawAuraborosPreDigest::Babe(RawBabePreDigest { authority_index, .. }) =>
						authority_index,
					RawAuraborosPreDigest::Aura(AuraPreDigest { slot_number, .. }) =>
						(slot_number % Authorities::get().len() as u64) as u32,
				});
			}
		}
		return None;
...
impl<T: Trait> Module<T> {
	...
	fn deposit_consensus<U: Encode>(new: U) {
		let log: DigestItem<T::Hash> = DigestItem::Consensus(AURABOROS_ENGINE_ID, new.encode());
