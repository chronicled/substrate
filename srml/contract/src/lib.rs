...
impl<T: Trait> Module<T> {
	...
	pub fn call_internal(
		...
		gas_limit: T::Gas,
		data: Vec<u8>,
	) -> rstd::result::Result<exec::CallReceipt, &'static str> {
...
macro_rules! m {
	( $name:ident, $ty:ty ) => {
		...
		fn derp() {
			...
			type BalanceOf<T> = <<T as contract::Trait>::Currency as support::traits::Currency<
				<T as system::Trait>::AccountId,
			>>::Balance;

			#[derive(Encode)]
			struct Params<T: contract::Trait> {
				origin: T::AccountId,
				dest: T::AccountId,
				value: BalanceOf<T>,
				gas_limit: T::Gas,
			...
			let params = Params::<$ty> {
				origin: AccountKeyring::Alice.into(),
				dest: AccountId::from_ss58check("5FxGtGMe9ZevHGDrvQqQYAB6zdc9s39Nuno7u3xQSd5Az5i8").unwrap(),
				value: 100,
				gas_limit: 500_000,
				data: vec![],
			};

			panic!("{}", substrate_primitives::hexdisplay::HexDisplay::from(&params.encode()));
		}

		#[no_mangle]
		fn $name(input_ptr: *const u8, input_len: usize) -> u64 {
			...
			let input = if input_len == 0 {
				// Make `input_len` a special case for not dealing with
				// alignment and being non-null requirements for `input_ptr`.
				&[]
			} else {
				unsafe { rstd::slice::from_raw_parts(input_ptr, input_len) }
			};

			let params = Params::<Runtime>::decode(&mut &input[..]).expect("failed to decode");

			let output = contract::Module::<Runtime>::call_internal(
				params.origin,
				params.dest,
				params.value,
				params.gas_limit,
